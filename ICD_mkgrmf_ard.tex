...
%    \leavevmode{\scalebox{0.17}{\includegraphics{asc_logo.eps}}}
    {\epsfxsize=3.2cm \leavevmode\epsfbox{asc_logo.eps}}
    \vspace*{0.1in}
...
%
%\item\label{appdoc:fitsdef}
% NOST 100-1.1, Definition of the Flexible Image Transport System
%                    (FITS)\newline
%                    {\tt http://www.cv.nrao.edu/fits/}
%
...
$$ G(r) = {1. \over (2\pi\sigma^2)} e^{{-(r-r_0)^2\over
(2\sigma^2)}}$$

where $\sigma$ is the Gaussian width in angstroms and $r_0$ is the
peak of the LSF. 


$$ L(r) = {L_0\over 2\pi} {\Gamma \over (r - r_0)^2 + \Gamma^2)} $$
where , r$_0$ is the peak position of the Lorenztian, which need not
be the same as the center of the Gaussian, and $\Gamma$ is the
half-width of the line profile. 
